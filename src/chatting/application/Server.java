...
public class Server extends JFrame {
    ...
    private void startServer(){    
        try{
            s = new ServerSocket(4044);
            client = s.accept();

            in = new BufferedReader(new InputStreamReader(client.getInputStream()));
            sent = new PrintWriter(client.getOutputStream(),true);
    ...
    private void header(){
        ...
        //profile
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icons/profile2.png")); //File path as we are  already inside src so start from there
        ...
        back.addMouseListener(new MouseAdapter(){        //Adding mouse Event
            public void mouseClicked(MouseEvent a){
                try {
                    // Close the socket and any other resources here
                    if (client != null) {
                        client.close();
                    }
                    if (s != null) {
    ...
    public void addSentMessage(String out){
